...
export default function Navbar({ user, activeSection, setActiveSection, onLogout }: NavbarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar navigation */}
      <aside className={`
        fixed top-14 lg:top-0 bottom-0 left-0 z-40 w-64 bg-slate-900 border-r border-slate-800 p-6 flex flex-col justify-between transition-transform duration-300 lg:translate-x-0
      ...
      `}>
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`
                    w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all group border-l-2
                    ${isActive 
        ...
        <div className="pt-6 border-t border-slate-800">
          <button
            id="nav-logout-btn"
            onClick={onLogout}
            className="w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-rose-400 hover:bg-rose-500/5 transition-all"
          >
